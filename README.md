...
### 2.4 边缘网关
#### 2.4.1 当前网关：RK3568
- 通信方式：通过串口或以太网与主控STM32F429连接，按部署方案完成协议适配
- 功能：将主控汇聚的传感器数据、告警状态通过MQTT协议上传至OneNET云平台
- 扩展能力：支持本地数据缓存、边缘计算（复杂逻辑处理）、多协议转换（MQTT/HTTP/CoAP）
...
- **通讯任务**：
  - 与RK3568交互：封装数据格式（设备ID、时间戳、传感器值、报警状态），通过串口或以太网发送
...
### 3.4 云端与应用层
#### 3.4.1 云平台（OneNET）
- 数据接入：接收RK3568上传的MQTT消息，存储原始数据（保留30天）
...
- **交互功能**：
  ...
  - 阈值设置：修改后同步至主控STM32F429（通过云端→RK3568→主控链路）
...
- **硬件系统**：
  ...
  - 边缘网关RK3568稳定上传数据至ONE net平台，通信成功率≥99%
...
- **系统集成**：
  ...
  - 预留扩展从机的硬件接口，并为RK3568侧协议适配与功能扩展保留空间
